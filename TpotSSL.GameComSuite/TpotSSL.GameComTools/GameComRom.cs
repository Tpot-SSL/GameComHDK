...
namespace TpotSSL.GameComTools {
    ...
    public class GameComRom :IDisposable{
        ...
        public static string GetGameName(ushort game){
            return KnownGamesById.ContainsKey(game) ? GetGameName(KnownGamesById[game]) : "Unknown  ";
        }
        ...
        public void UpdateHeader() {
            ...
            var nameBytes = Encoding.ASCII.GetBytes(GameName);
        ...
        public void ReplaceBank(int bankIndex, byte[] bytes, Bitmap image){
            var bank = MemoryBanks[bankIndex];
            using(var g = Graphics.FromImage(FullImage.Image))
        ...
        public void GrabHeaderData(int index){
            ...
            if(IconBankNo < MemoryBanks.Count)
                GameIcon = MemoryBanks[IconBankNo].Image.Image.Clone(new Rectangle(IconX, IconY, 64, 64), System.Drawing.Imaging.PixelFormat.Format32bppArgb);
